...
The terminal client only needs three MCP operations: initialize the session,
discover tools, and call one selected tool. This module keeps that flow explicit
so Upwind developers can see the exact JSON-RPC messages and authentication
...
class SentinelMCPClient:
    """Async Sentinel MCP client used by the local terminal runner."""
    ...
    def __init__(
        ...
        *,
        ...
        # The client can use either a collection name or a full server URL. The
    ...
    async def _get_token(self) -> str:
        ...
        if tenant_id and client_id and client_secret:
            # Service-principal auth is useful for unattended runs or hosted apps.
    ...
    async def _send_request(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._http.stream("POST", f"{self.server_url}/", json=payload) as response:
            ...
            if "text/event-stream" in content_type:
                # Sentinel MCP may stream JSON-RPC results over SSE, so the client
